...
void vulkanAPI::setup(Window &window)
{
    this->window = &window;
    createInstance();
    setupDebugMessenger();
...
void vulkanAPI::drawFrame()
{
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || window->pFramebufferResized) {
...
void vulkanAPI::createInstance()
{
    ...
    if (enableValidationLayers) {
        ...
        struct CreateInfoPNext {
            VkDebugUtilsMessengerCreateInfoEXT debugCreateInfo;
            VkValidationFeaturesEXT features;
        };

        CreateInfoPNext createInfoPNext;
        createInfoPNext.features = features;
        createInfoPNext.debugCreateInfo = debugCreateInfo;
...
void vulkanAPI::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLevels, VkSampleCountFlagBits numSamples)
{
